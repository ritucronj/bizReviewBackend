...
const getAllUsers = async (req, res) => {
    try {
        ...
        const reviewerData = new Promise(async (resolve, reject) => {
            resolve(await reviewer.find(queryFilter).sort({ name: 1 }).limit(limit * 1).skip((page - 1) * limit).select({ _id: 0, name: 1, email: 1, createdAt: 1, userType: 1, isDeleted: 1 }));
        });
        const buisnessData = new Promise(async (resolve, reject) => {
            resolve(await buisness.find(queryFilter).sort({ companyName: 1 }).limit(limit * 1).skip((page - 1) * limit).select({ _id: 0, companyName: 1, email: 1, createdAt: 1, userType: 1, isDeleted: 1 }));
        });
