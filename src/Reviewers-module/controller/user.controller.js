...
const { userRegistrationValidation, ssoRegistrationValidation, userUpdateValidataion } = require("../services/Validation-handler");
            ...
            if (email_verified) {
                const saveUserData = new Promise(async (resolve, reject) => {
                    ...
                    if (registeredUser) {
                        ...
                        resolve(await user.create({ name: name, email: email, profilePicture: picture, language: locale }));
                    }
                });
                saveUserData
                    .then((data) => {
...
const updateUserProfile = async (req, res) => {
    try {
        ...
        const updatePromise = new Promise(async (resolve, reject) => {
            ...
            const findUser = await user.findOneAndUpdate({ uId: userId }, req.body, { new: true });
...
const deleteUserProfile = async (req, res) => {
    try {
        ...
        const deletePromise = new Promise(async (resolve, reject) => {
            const findUser = await user.findOneAndUpdate({ uId: userId }, { isDeleted: true }, { new: true });
            if (findUser === null) reject(new Error("No User Found"));
            else resolve();
        });
        deletePromise
            .then(async () => {
                await review.findOneAndUpdate({ createdBy: userId }, { isUserActive: false }, { new: true });
                return res.status(statusCodes[200].value).send({ msg: "Successfully Deleted" });
            })
    } catch (error) {
        return res.status(statusCodes[500].value).send({ msg: error.message });
    }
